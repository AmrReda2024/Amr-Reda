...
| `JS-Law-Firm-Profile-A4.pdf` | 11-page A4 print build, no bleed. The version to email or print on a desk printer. |
| `JS-Law-Firm-Profile-A4-bleed.pdf` | Same 11 pages at A4 trim plus 3 mm bleed and corner crop marks, for a commercial printer. |
...
- **The profile answers a question before it asks for anything.** "Which gates apply to
  you" takes six yes/no answers and returns the gates that bind, in order, with the
  authority and instrument for each. Where a transaction is involved it applies the
  published merger-control thresholds arithmetically (EGP 900 m / 200 m domestic,
  EGP 7.5 bn global) and says which test is met on the figures entered. Everything runs
  in the browser; nothing is sent anywhere.
- **Take-away.** "Copy as a memo" puts the result on the clipboard as plain text a general
  counsel can paste into an internal email, dated and attributed. Page 5 of the print build
  is the same six questions as a paper worksheet with the threshold arithmetic set out.
- **Deep links.** Each gate has its own anchor — `.../index.html#gate-3` opens gate 3 and
  scrolls to it, so one gate can be sent to a colleague without a covering explanation.
